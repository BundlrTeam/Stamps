...
try {
  ...
  console.log('Módulo "pg" não encontrado. A instalar...');
  const { execSync } = require('child_process');
  try {
    execSync('npm install pg --no-save', { stdio: 'inherit' });
    Client = require('pg').Client;
  } catch (err) {
    console.error('Erro ao instalar o módulo "pg":', err);
    process.exit(1);
  }
}

// Ler as variáveis do ficheiro .env para obter SUPABASE_URL e SUPABASE_KEY
let env = {};
try {
  ...
  console.error('Erro ao ler .env no setup-db.js:', e.message);
}
...
if (!connectionString) {
  console.error('Erro: Nenhuma connection string (STRING_SQLSUPA) fornecida.');
  process.exit(1);
}
...
const baseMocks = [
  ...
  },
  {
    ...
    description: 'Mercearia de bairro com produtos frescos, cestas semanais e focus em produtores locais.',
    category: 'Loja',
...
console.log('A ligar à base de dados do Supabase...');
client.connect()
  .then(() => {
    console.log('Ligação estabelecida. A executar script SQL de criação das tabelas...');
    return client.query(sqlCreateTables);
  })
  .then(() => {
    console.log('Tabelas businesses e merchant_leads verificadas/criadas com sucesso no Supabase.');
    console.log('A verificar se a tabela businesses necessita de seeding...');
    return client.query('SELECT COUNT(*) FROM businesses');
  })
  .then(async (resCount) => {
    const count = parseInt(resCount.rows[0].count, 10);
    if (count === 0) {
      console.log('A tabela businesses está vazia. A iniciar seeding dos 13 negócios padrão...');
      for (const biz of baseMocks) {
        const image = resolveImageUrl(biz.image, `businesses/${biz.id}/main.jpg`);
        const images = biz.images.map((img, idx) => resolveImageUrl(img, `businesses/${biz.id}/gallery${idx === 0 ? '1' : idx + 1}.jpg`));
        const logo = resolveImageUrl(biz.logo, `businesses/${biz.id}/logo.jpg`);
        ...
        const insertSql = `
          INSERT INTO businesses (
            ...
            reward, reward_description, qr_code_pattern, card_customization, approved_at
  ...
  .then(() => {
    console.log('Processo de setup e seeding finalizado.');
  })
  .catch(err => {
    console.error('Erro ao executar setup do banco de dados no Supabase:', err.message);
    try {
      client.end();
    } catch (e) {}
    process.exit(1);
